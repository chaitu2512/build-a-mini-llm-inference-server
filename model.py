...
def stable_softmax(logits):
    ...
    # print(exp_logits)
    exp_stable_logits = np.exp(stable_logits)
    sum_exp_stable_logits = np.sum(exp_stable_logits, axis = -1, keepdims = True)
    return exp_stable_logits/sum_exp_stable_logits
...
def build_vocab(corpus, special_tokens):
    ...
    for special_token in special_tokens:
        token_to_id_map[special_token] = count
        id_to_token_map.append(special_token)
        count+=1
        seen.add(special_token)

    elongated_string = "".join(corpus)
    elongated_string = sorted(elongated_string)
    for c in elongated_string:
        if c not in seen:
            token_to_id_map[c] = count
            id_to_token_map.append(c)
            count+=1
            seen.add(c)
    vocab = {}
    vocab['token_to_id'] = token_to_id_map
    vocab['id_to_token'] = id_to_token_map
    return vocab
...
def encode_prompt(text, vocab, add_bos=True):
    ...
    encoded_prompt = []
    if add_bos:
        encoded_prompt = [vocab['token_to_id']["<bos>"]]
    
    for c in text:
        if c in vocab['token_to_id']:
            encoded_prompt.append(vocab['token_to_id'][c])
        else:
            encoded_prompt.append(vocab['token_to_id']["<unk>"])
